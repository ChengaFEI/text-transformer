...
def evaluate_perplexity(model: Any, data: Any):
    ...
    Inputs:
        ...
        data (Any):  Data in the form suitable for the input model. For the n-gram model, data should be of type List[List[Any]]. For the transformer, the data should by of type torch.utils.data.DataLoader.
    """  # noqa: E501
    m = num_tokens_in_corpus(model, data)
    log = corpus_log_probability(model, data)
    return 2 ** (- log / m)
...
def corpus_log_probability(model, data):
    ...
    if type(data) is list:
        ...
        for input_tokens, target_tokens in data:
            ...
            log_p += torch.sum(model.log_probability(
